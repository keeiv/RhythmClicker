...
namespace ClickerGame
{
    public class Game1 : Game
    {
        ...
        // Menu / scene
        enum GameState { Menu, Playing, Result, Account, Language, Stats, BeatmapEditor, Settings, Achievements, ReplayView, Profile, SearchPlayer }
  ...
  { ""Id"": ""ba_midsummer"", ""Title"": ""Midsummer Daydream"", ""File"": ""ba_midsummer.wav"", ""Difficulties"": [""easy"", ""hard"", ""difficulty"", ""very_difficulty""] }
        ...
        void DrawProfile()
        {
            ...
            // Avatar placeholder (circle-ish)
            int avSize = 56;
            int avX = cardX + 24, avY = cardY + bannerH - avSize / 2;
            spriteBatch.Draw(pixel!, new Rectangle(avX - 1, avY - 1, avSize + 2, avSize + 2), new Color(0, 200, 255) * 0.4f);
            spriteBatch.Draw(pixel!, new Rectangle(avX, avY, avSize, avSize), new Color(20, 20, 40));
            var avLetter = textRenderer!.GetTexture(p.User.Length > 0 ? p.User[0].ToString().ToUpper() : "?", "Segoe UI", 28, new Color(0, 200, 255));
        ...
        void DrawSearchPlayer()
        {
            ...
            else
            {
                for (int i = 0; i < searchResults.Count; i++)
                {
                    ...
                    // Avatar placeholder
                    int avS = 36;
                    spriteBatch.Draw(pixel!, new Rectangle(sx + 8, ry + 9, avS, avS), new Color(30, 30, 50));
                    var avLetter = textRenderer!.GetTexture(r.Username.Length > 0 ? r.Username[0].ToString().ToUpper() : "?", "Segoe UI", 18, new Color(0, 200, 255));
